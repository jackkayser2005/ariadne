...
func TestRunBrowserCapture(t *testing.T) {
	...
	capture := func(procedure, driver, output string) (browser.CaptureSummary, error) {
		if procedure != "procedure.json" || driver != "driver.exe" || output != "trace.json" {
			t.Fatalf("capture args = %q, %q, %q", procedure, driver, output)
		}
		return want, nil
	}
	var stdout, stderr bytes.Buffer
	if exitCode := runBrowserCapture([]string{"--procedure", "procedure.json", "--driver", "driver.exe", "trace.json"}, &stdout, &stderr, capture); exitCode != 0 {
	...
	stdout.Reset()
	stderr.Reset()
	if exitCode := runBrowserCapture([]string{"--json", "--procedure", "procedure.json", "--driver", "driver.exe", "trace.json"}, &stdout, &stderr, capture); exitCode != 0 {
	...
	stdout.Reset()
	stderr.Reset()
	failing := func(string, string, string) (browser.CaptureSummary, error) {
