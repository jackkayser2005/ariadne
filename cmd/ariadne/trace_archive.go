...
func runTraceArchiveCreate(args []string, stdout, stderr io.Writer, save traceArchiveSaver) int {
	flags := flag.NewFlagSet("trace archive create", flag.ContinueOnError)
	flags.SetOutput(io.Discard)
	jsonOutput := flags.Bool("json", false, "")
	var tracePaths, sessionPaths []string
	...
	if err := flags.Parse(args); err != nil || flags.NArg() != 1 || len(tracePaths) == 0 || len(tracePaths) != len(sessionPaths) {
...
func writeTraceArchiveSummary(stdout io.Writer, heading string, summary trace.ArchiveVerificationSummary) error {
	if _, err := fmt.Fprintf(stdout, "%s\norder_basis: %s\nentries: %d\ncomplete: %d\npartial: %d\narchive_sha256: %s\n", heading, summary.OrderBasis, summary.Entries, summary.Complete, summary.Partial, summary.ArchiveSHA256); err != nil {
		return err
	}
