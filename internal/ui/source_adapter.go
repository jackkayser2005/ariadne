...
<section class="panel" id="observed-paths"><p class="eyebrow">Observed paths</p><h2>Where the safe labels appeared</h2><p>Read each path as source → reviewed category → destination. These are labels only; the original payloads and values are not here.</p><div>{{range .TraceEvents}}<ol class="path" aria-label="Recorded source-adapter path"><li><span class="path-label">Source</span><strong>{{.Source}}</strong><small>{{.Channel}} / {{.Kind}}</small></li><li><span class="path-label">Reviewed category</span><div class="path-tags">{{range .Fields}}<span class="tag">{{.}}</span>{{end}}</div><small>label retained by the verifier</small></li><li><span class="path-label">Destination</span><strong>{{.Destination}}</strong><small>recorded boundary</small></li></ol>{{else}}<p class="unknown">No safe event paths were retained.</p>{{end}}</div></section>
