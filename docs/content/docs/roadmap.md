...
The loopback review surface now exposes one verified run at /source-adapter
with the same raw-value-free receipt, trace, session, completeness, and replay
limitations.
...
session receipts use schema 9 and replication roots use schema 2. The golden
Android acceptance receipt now uses schema 2 and records the shared
`environment_sha256` identity only after the standalone run and two-order
replication agree on that environment. A session binding covers safe target identity, reset policy, manifest and procedure
...
**Current tiered validation slice.** `ariadne validate [--json] <artifact>` now composes the existing manifest, Android replication, Android minimization, trace archive, trace replication, trace case, trace study, browser weather, generic source-adapter, and bounded HAR verifiers into one raw-value-free report. It identifies the artifact family and returns four fixed tiers: structural schema validity, canonical-content and child-artifact integrity, authenticated provenance and execution-boundary consistency, and replay/reproducibility readiness. Android replication and minimization reach boundary pass only when authenticated bindings cover the checked sessions and ordered pairs; a provenance digest alone remains insufficient. Current Android reports also expose the shared `environment_sha256` binding identity as safe provenance, while legacy artifacts omit it. The last tier is a readiness check only; validation never launches an adapter or device. `pass`, `warning`, `unknown`, `fail`, and `unavailable` remain distinct: legacy artifacts can be structurally valid while provenance is unavailable, and incomplete captures remain unknown. The report's counterfactual outcome, where present, remains separate from `evidence_state`, and no report status makes a causal claim. The initial surface recognizes JSON experiment manifests, Android `replication.json` and `minimization.json` roots, verified source-neutral trace archives, replication ledgers, cross-source cases, and replication studies, plus standalone Android evidence-run directories, the verified browser weather and generic source-adapter run directories, and bounded HAR export files; verified browser fixture replication and minimization directories plus proxy replication directories now join the common validator; standalone redacted trace documents now join the common validator with provenance unavailable when they lack a session envelope; raw-value-free Android acceptance receipts now join it as contract-only checks with replay unavailable; schema 2 receipts expose the joined environment identity, while legacy schema 1 receipts remain readable with their authenticated boundary unavailable. Standalone Android runs preserve legacy readability while reporting boundary unavailable unless both sessions carry the current authenticated binding; malformed or ambiguous run directories fail closed;  archive-question reflections and transition histories now join as contract-only checks with evidence state unknown and replay unavailable; portable fixed-question rounds, selected receipts, and acceptance records now join as contract-only checks with source provenance unavailable and replay unavailable; HAR exports remain on their specialized verifier until their own mapping slice is reviewed. At the CLI, only an aggregate pass exits successfully; every other aggregate status is nonzero so unavailable evidence cannot be mistaken for a clean validation.
...
and publishes only safe identities, counts, the joined environment identity,
and fixed review metadata. A v2 receipt is written only when the standalone
run and authenticated replication share the same non-empty environment digest;
legacy v1 receipts remain readable without inheriting that boundary guarantee. Offline
...
in the current line. The verified browser fixture replication and minimization directories plus proxy replication directories and standalone redacted trace documents now participate in that common validation surface; Android acceptance receipts and archive-question reflections, transition histories, fixed question rounds, selected receipts, and acceptance records now participate as contract-only checks with replay unavailable; emulator evidence, wider artifact-family mapping, and
