...
**Current tiered validation slice.** `ariadne validate [--json] <artifact>` now composes the existing manifest, Android replication, Android minimization, trace archive, trace replication, trace case, trace study, browser weather, generic source-adapter, and bounded HAR verifiers into one raw-value-free report. It identifies the artifact family and returns four fixed tiers: structural schema validity, canonical-content and child-artifact integrity, authenticated provenance and execution-boundary consistency, and replay/reproducibility readiness. Android replication and minimization reach boundary pass only when authenticated bindings cover the checked sessions and ordered pairs; a provenance digest alone remains insufficient. The last tier is a readiness check only; validation never launches an adapter or device. `pass`, `warning`, `unknown`, `fail`, and `unavailable` remain distinct: legacy artifacts can be structurally valid while provenance is unavailable, and incomplete captures remain unknown. The report's counterfactual outcome, where present, remains separate from `evidence_state`, and no report status makes a causal claim. The initial surface recognizes JSON experiment manifests, Android `replication.json` and `minimization.json` roots, verified source-neutral trace archives, replication ledgers, cross-source cases, and replication studies, plus the verified browser weather and generic source-adapter run directories, and bounded HAR export files; verified browser fixture replication and minimization directories plus proxy replication directories now join the common validator; standalone redacted trace documents now join the common validator with provenance unavailable when they lack a session envelope; raw-value-free Android acceptance receipts now join it as contract-only checks with replay unavailable; archive-question reflections and transition histories now join as contract-only checks with evidence state unknown and replay unavailable; portable fixed-question rounds, selected receipts, and acceptance records now join as contract-only checks with source provenance unavailable and replay unavailable; HAR exports remain on their specialized verifier until their own mapping slice is reviewed. At the CLI, only an aggregate pass exits successfully; every other aggregate status is nonzero so unavailable evidence cannot be mistaken for a clean validation.
